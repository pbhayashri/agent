...
use AssetSync::Agent::Task::Inventory::HPUX::Networks;
...
foreach my $test (keys %lanadmin_tests) {
    ...
    my $info = AssetSync::Agent::Task::Inventory::HPUX::Networks::_getLanadminInfo(file => $file);
...
foreach my $test (keys %ifconfig_tests) {
    ...
    my $info = AssetSync::Agent::Task::Inventory::HPUX::Networks::_getIfconfigInfo(file => $file);
...
foreach my $test (keys %nwmgr_tests) {
    ...
    my $info = AssetSync::Agent::Task::Inventory::HPUX::Networks::_getNwmgrInfo(file => $file);
    cmp_deeply($info, $nwmgr_tests{$test}, "nwmgr parsing: $test");
}

foreach my $test (keys %netstat_tests) {
    my $file = "resources/hpux/netstat/$test";
    my %interfaces = AssetSync::Agent::Task::Inventory::HPUX::Networks::_parseNetstatNrv(file => $file);
...
foreach my $test (keys %lanscan_tests) {
    ...
    my @interfaces = AssetSync::Agent::Task::Inventory::HPUX::Networks::_parseLanscan(file => $file);
