...
use AssetSync::Agent::Task::Inventory::Generic::USB;
...
foreach my $test (keys %lsusb_tests) {
    ...
    my @devices = AssetSync::Agent::Task::Inventory::Generic::USB::_getDevicesFromLsusb(file => $file);
    cmp_deeply(\@devices, $lsusb_tests{$test}, "$test: lsusb parsing");
}

foreach my $test (keys %usb_tests) {
    my $file = "resources/generic/lsusb/$test";
    my @devices = AssetSync::Agent::Task::Inventory::Generic::USB::_getDevices(file => $file, datadir => './share');
