...
use AssetSync::Agent::Task::Inventory::AIX::LVM;
...
foreach my $test (keys %physical_volume_tests) {
    ...
    my $device = AssetSync::Agent::Task::Inventory::AIX::LVM::_getPhysicalVolume(file => $file, name => $test);
...
foreach my $test (keys %logical_volume_tests) {
    ...
    my $device = AssetSync::Agent::Task::Inventory::AIX::LVM::_getLogicalVolume(file => $file, name => $test);
...
foreach my $test (keys %volume_group_tests) {
    ...
    my $device = AssetSync::Agent::Task::Inventory::AIX::LVM::_getVolumeGroup(file => $file, name => $test);
