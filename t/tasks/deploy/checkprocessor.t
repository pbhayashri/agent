...
sub base_object {
    ...
    my $class = "AssetSync::Agent::Task::Deploy::CheckProcessor";
...
my %win32_regs = (
    ...
    'winkey-exists-4' => {
        type      => "winkeyExists",
        path      => 'HKEY_LOCAL_MACHINE\SOFTWARE\AssetSync-Agent',
        __expect  => base_object("WinKeyExists", {
            type   => "winkeyExists",
            path   => 'HKEY_LOCAL_MACHINE\SOFTWARE\AssetSync-Agent'
        }),
    ...
    'winkey-exists-5' => {
        type      => "winkeyExists",
        path      => 'HKEY_LOCAL_MACHINE\SOFTWARE\AssetSync-Agent\debug3',
        __expect  => base_object("WinKeyExists", {
            type   => "winkeyExists",
            path   => 'HKEY_LOCAL_MACHINE\SOFTWARE\AssetSync-Agent\debug3'
        }),
    ...
    'winkey-value-9' => {
        type      => "winkeyEquals",
        path      => 'HKEY_LOCAL_MACHINE\SOFTWARE\AssetSync-Agent\debug',
        value     => undef,
        __expect  => base_object("WinKeyEquals", {
            type   => "winkeyEquals",
            path   => 'HKEY_LOCAL_MACHINE\SOFTWARE\AssetSync-Agent\debug',
    ...
    'winvalue-type-8' => {
        type      => "winvalueType",
        path      => 'HKEY_LOCAL_MACHINE\SOFTWARE\AssetSync-Agent\debug4/',
        value     => "REG_SZ",
        __expect  => base_object("WinValueType", {
            type   => "winvalueType",
            path   => 'HKEY_LOCAL_MACHINE\SOFTWARE\AssetSync-Agent\debug4/',
    ...
    'winvalue-type-9' => {
        type      => "winvalueType",
        path      => 'HKEY_LOCAL_MACHINE\SOFTWARE\AssetSync-Agent\debug2',
        value     => "REG_DWORD",
        __expect  => base_object("WinValueType", {
            type   => "winvalueType",
            path   => 'HKEY_LOCAL_MACHINE\SOFTWARE\AssetSync-Agent\debug2',
...
my $diskFreeModule = Test::MockModule->new(
    'AssetSync::Agent::Task::Deploy::DiskFree'
);
$diskFreeModule->mock(
    'getFreeSpace', sub {
        return 200;
...
my $regdump = loadRegistryDump("resources/win32/registry/assetsync-agent-audit-test.reg");
    ...
    '_getRegistryKey', sub {
        ...
        if ( $params{root} && $params{root} =~ m|^HKEY_LOCAL_MACHINE| ) {
            ...
            return $regdump->{$params{keyName}.'/'}
                if ($params{keyName} eq 'AssetSync-Agent');
            return $regdump->{'AssetSync-Agent/'}->{$params{keyName}.'/'}
                if ($params{keyName} ne '/' && $params{root} =~ 'AssetSync-Agent$');
        }
        return {};
    }
);
