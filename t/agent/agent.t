...
push @INC, $libdir;
my $agent = AssetSync::Agent->new(libdir => $libdir);

# No agentid set by default
ok(!exists($agent->{agentid}));

create_file("$libdir/AssetSync/Agent/Task/Task1", "Version.pm", <<'EOF');
package AssetSync::Agent::Task::Task1::Version;
...
create_file("$libdir/AssetSync/Agent/Task/Task2", "Version.pm", <<'EOF');
package AssetSync::Agent::Task::Task2::Version;
use constant VERSION => 42;
1;
EOF
cmp_deeply (
...
create_file("$libdir/AssetSync/Agent/Task/Task3", "Version.pm", <<'EOF');
package AssetSync::Agent::Task::Task3::Version;
use Does::Not::Exists;
use constant VERSION => 42;
1;
EOF
...
create_file("$libdir/AssetSync/Agent/Task/Task5", "Version.pm", <<'EOF');
package AssetSync::Agent::Task::Task5::Version;
use constant VERSION => 42;
1;
EOF
cmp_deeply (
...
# Setup agent
$agent->{config} = AssetSync::Agent::Config->new(
    options => {
        config  => 'none',
        debug   => 1,
        logger  => 'Test'
...
$agent->{logger} = AssetSync::Agent::Logger->new(config => $agent->{config});
...
sub checktasksplan {
    ...
    my @tasksExecutionPlan = AssetSync::Agent::_makeExecutionPlan(\@taskinconf, $availableTasks);
